 ...
 * ==========================================================
 *
 * hacked by ronaldoooo to fit bootstrap v3
 * 1. use chinese messages
 * 2. change the wrapper tag from <label> to <div>, which is usually used in bootstrap v3
 * 3. change the class 'error' to 'has-error'
 * 4. change the error label, add class 'control label'
 * ========================================================== */
...
;(function($, undefined) {
	...
	$.extend(Valid.prototype, {
		...
		show: function(errors) {
			...
			$.each(errors, function(k, v) {
				...
				if ($span.length === 0) {
					$span = $('<span/>', {'class': 'message form-control-feedback'});
					$input.after($span);
				}
				$span.html(v);
			});
		...
		messages: {
			not_empty: '请填写这个字段.'
			,min_length: '要求至少 :value 个字符.'
			,max_length: '要求之多 :value 个字符.'
			,regex: ''
			,email: '请填写正确的email地址.'
			,url: '请填写一个正确的url地址.'
			,exact_length: '要求恰好 :value 个字符.'
			,equals: ''
			,ip: ''
			,credit_card: '请填写一个正确的身份证号.'
			,alpha: ''
			,alpha_numeric: ''
			,alpha_dash: ''
	...
	$.fn[pluginName].defaults = {
		wrapper: 'div'	// the html tag that wraps the field and which defines a "row" of the form
